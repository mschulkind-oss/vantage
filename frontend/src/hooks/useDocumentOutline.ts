...
export interface OutlineEntry {
  ...
  level: number;
  /**
   * The element to scroll to. Held rather than re-resolved, because a question
   * may have no id to re-resolve by, and because the list is rebuilt on every
   * mutation anyway — a reference that has gone stale fails the `offsetParent`
   * test in [measureEntryOffsets] exactly as a missing id would.
   */
  element: HTMLElement;
}
