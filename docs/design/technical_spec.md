...
- `host`, `port`: server bind settings (defaults `127.0.0.1`, `8000`). The
  port is a starting point, not a requirement: the listeners are opened
  before the server runs, and if the port is taken the first free one above
  it is used for every bind address, up to 100 ports above the one
  configured — past that, startup fails rather than silently picking
  something arbitrary. Every bind address shares one port, so a port free on
