...
- `watcher_ignore_defaults`: live-reload watcher-only gitignore patterns
  prepended before user and workspace ignore files. Absent ⇒ the built-in
  generated/dependency-tree defaults; present (even empty) ⇒ replace them.
...
- **`live.Manager` + `live.Watcher`** — the realtime layer.
  ...
  - The `Watcher` (fsnotify) coalesces filesystem events, invalidates the
    ...
    and broadcasts a sorted `files_changed` payload. It prunes `.git`
    descendants except the top level, prunes Vantage-owned `.vantage` state
    except `.vantage/inbox`, and applies `watcher_ignore_defaults` before user
    and workspace ignore files. `exclude_dirs` is listing-only and does not
    affect watcher registration.
